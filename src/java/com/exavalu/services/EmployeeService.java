...
public class EmployeeService {
    ...
    public static boolean RetrieveDeletedEmployee(Employee emp) {

        boolean result = false;
        try {
            Connection con = JDBCConnectionManager.getConnection();
            String sql = "UPDATE employeedb.employees SET isDeleted = 0 WHERE employeeId = ?;";
...
//            System.out.println("size of rs="+ rs.getFetchSize());
     ...
     //TO SEARCH IN DELETED EMPLOYEE
     public ArrayList doSearchDeletedEmployee(Employee emp) {
            
        ArrayList empList = new ArrayList();
        try {
            Connection con = JDBCConnectionManager.getConnection();
            String sql = "select * from employees e, departments d, role r where e.departmentId=d.departmentId and e.roleId=r.roleId having firstName like ? and lastName like ? and gender like ? and departmentName like ? and roleName like ? AND isDeleted=1;";
