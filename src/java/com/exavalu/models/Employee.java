...
//             sessionMap.put("RoleList", roleList);
            ...
            sessionMap.put("UpdateMsg", createdMsg);
            ArrayList empList = new ArrayList();
            empList = EmployeeService.getInstance().getAllEmployees();
            sessionMap.put("EmpListHome", empList);
    ...
    public String doRetrieve() throws Exception {
        ...
        String result = "FAILURE";
        boolean success = EmployeeService.RetrieveDeletedEmployee(this);

        if (success) {
            String updateMsg = "employeeId :: " + this.getEmployeeId() + " :: Restored successfully!";
            sessionMap.put("UpdateMsg", updateMsg);
    ...
    public String doSearchDeleted() throws Exception {

        String result = "FAILURE";
        ArrayList empList = EmployeeService.getInstance().doSearchDeletedEmployee(this);

        //System.out.println("depList size::"+depList.size());
        System.out.println("emplist size::" + empList.size());
        if (empList.size()!= 0) {

            sessionMap.put("EmpListDeletePage", empList);
            sessionMap.put("NoResultMsg", null);

            System.out.println("returning Success from doSearch method");
            result = "SUCCESS";

        } else {
            String noResultMsg = "No such Employee Exist!!";
            sessionMap.put("EmpListDeletePage", null);
            sessionMap.put("NoResultMsg", noResultMsg);
            result = "SUCCESS";
        }
        return result;
    }

    public String dogoToHome() throws Exception {
